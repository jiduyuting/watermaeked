...
import numpy as np
from model import ResNet18, vgg19_bn,TriggerAppending
...
def load_trigger_alpha(trigger_path, alpha_path):
    from PIL import Image
    trigger = transforms.ToTensor()(Image.open(trigger_path))
    alpha = transforms.ToTensor()(Image.open(alpha_path))
    assert (torch.max(trigger) < 1.001) and (torch.max(alpha) < 1.001)
...
def main():
    ...
    trigger, alpha = load_trigger_alpha(args.trigger, args.alpha)
